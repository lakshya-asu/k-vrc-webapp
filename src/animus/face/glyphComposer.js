// src/animus/face/glyphComposer.js
// The generative glyph composer (reel-polish brief, directive 3).
//
// Reference 4's lesson: K-VRC's faces are COMPOSED GLYPHS, not fixed
// library entries. A face is eyes + brows + mouth picked from a small
// parametric vocabulary, or a short piece of LED text filling the
// screen (the WTF face). The actor brain may emit these through the
// face_glyph beat channel; everything here is strictly validated so a
// model can never push arbitrary drawing through the contract.
//
// Drawing shares the canvas, palette, and geometry constants of
// faceScreenDraw.js so composed faces sit exactly where library faces
// sit (the blink eyelid overlay and speech bob keep working).

import { MOOD_COLORS } from './faceScreenDraw.js';

// The face canvas size. Kept as local constants (not imported values)
// because faceScreenDraw imports this module: ESM resolves the cycle
// for run-time lookups like MOOD_COLORS, but top-level constant math
...
export function validateFaceGlyph(spec, path = 'face_glyph') {
  ...
  if (spec.mood !== undefined) {
    if (!Object.prototype.hasOwnProperty.call(MOOD_COLORS, spec.mood)) {
      errors.push(
        `${path}.mood must be one of: ${Object.keys(MOOD_COLORS).join(', ')}`,
      );
    } else {
...
// --- drawing --------------------------------------------------------------
// Shared geometry: identical eye/mouth anchors to drawWeighted so the
// blink overlay and the speech scale bob keep lining up.
const EY = H * 0.42;
...
function drawEye(ctx, kind, ex, blink, k) {
  ...
  switch (kind) {
    ...
    case 'bar':
      ...
      break;
    case 'happy_arc':
      // ^ ^ closed happy eyes (reference 4, face 2).
      ctx.beginPath();
  ...
  for (const side of [-1, 1]) {
    ...
    switch (kind) {
      ...
      case 'raised':
        ...
        break;
      case 'angry_in':
        // Inner ends slant down toward the nose (reference 4, face 3).
...
function drawMouth(ctx, kind, k, amplitude, c) {
  ...
  switch (kind) {
    ...
    case 'frown':
      ...
      break;
    case 'v_smile':
      // Small v smile (reference 4, face 1).
      ctx.beginPath();
    ...
    case 'o_small':
      ...
      break;
    case 'grin_rect': {
      // Big open grin: large rounded-square mouth (reference 4, face 4).
      const gw = 150 * k;
    ...
    case 'gritted': {
      // Gritted flat mouth with teeth separators (reference 4, face 3).
      const gw = 160 * k;
      const gh = 44 * k;
...
// c: the mood palette; blink 0..1; amplitude 0..1 speech loudness.
